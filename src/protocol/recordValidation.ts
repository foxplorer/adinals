...
import {
  ...
  type AdinalsApproval,
  type AdinalsFormat,
} from './collectionMetadata.ts'
import type { SpendLinkedRecordProof } from './transitions.ts'
...
/**
 * One indexed record as the reader sees it, independent of who holds it. The
 * raw-key application derived `owner` from a single permanent address; under
 * BRC-100 every record has its own derived address, so ownership is decided by
 * wallet custody and this row stays purely a protocol fact.
 */
...
export const readAdinalsSubTypeData = (map: AdinalsMap): AdinalsMap => {
  ...
  return raw && typeof raw === 'object' && !Array.isArray(raw) ? raw as AdinalsMap : {}
}
...
export const validateProtocolAdUrl = (value: unknown): { url: string; error: string } => {
  ...
  try {
    ...
    if (parsed.protocol !== 'https:') return { url: '', error: 'Destination URL must start with https://.' }
...
export const validateWritableProtocolAdUrl = (value: unknown): { url: string; error: string } => {
  ...
  return bytes <= ADINALS_URL_WRITE_MAX_BYTES
    ? result
    : { url: '', error: `Destination URL must be ${ADINALS_URL_WRITE_MAX_BYTES.toLocaleString()} UTF-8 bytes or fewer.` }
}

const commonRecordError = (row: AdinalsProtocolRow, subType: string, app: string): string => {
  if (
    row.map.app !== app ||
    row.map.type !== 'ord' ||
...
export const collectionRulesFromRecord = (
  ...
  app: string = ADINALS_NAMESPACE.app,
  ...
  const format: AdinalsFormat = row.map.adFormat === 'image' ? 'image' : 'text'
  const approval: AdinalsApproval = row.map.adApproval === 'open' ? 'open' : 'creator'
  ...
  let error = commonRecordError(row, ADINALS_SUB_TYPE.collection, app)
  ...
  } else if (!error && row.map.adContentPolicy !== undefined && row.map.adContentPolicy !== 'family-friendly') {
...
export const adMintRecordError = (
  ...
  const commonError = commonRecordError(row, ADINALS_SUB_TYPE.ad, app)
...
export const adUpdateRecordError = (
  ...
  context: {
    ...
    transition: SpendLinkedRecordProof
  },
  app: string = ADINALS_NAMESPACE.app,
): string => {
  const commonError = commonRecordError(row, ADINALS_SUB_TYPE.update, app)
  if (commonError) return commonError
...
export const adDecisionRecordError = (
  ...
  const commonError = commonRecordError(row, ADINALS_SUB_TYPE.decision, app)
  if (commonError) return commonError
